    ...
    <div className="galerie">
      {projectsData.map((project) => (
        <div key={project.id} className={`project ${expandedProjectId === project.id ? 'open' : ''}`}>
          <h3>{project.titre}</h3>
          <img src={`${process.env.PUBLIC_URL}/${project.image}`} alt={project.titre} />
          ...
          <button className="en-savoir-plus" onClick={() => toggleProjectInfo(project.id)}>
            {expandedProjectId === project.id ? 'Fermer' : 'En savoir plus'}
          </button>
